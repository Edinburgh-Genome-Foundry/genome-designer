...
/**
 * find the first parent that has the given class or null if not found
 * @param  {HTMLElement} el
 * @param  {String} klass
 * @return {HTMLElement}
 */
D.findParentWithClass = function(el, klass) {

  while (el) {
    // non HTML elements will not have a className string
    if (_.isString(el.className) && el.className.split(' ').indexOf(klass) >= 0) {
      return el;
    }
    el = el.parentElement;
  }
  // if here no parent with the class was found
  return null;
};
